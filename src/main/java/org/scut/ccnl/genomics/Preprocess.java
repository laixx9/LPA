...
public class Preprocess {
    ...
    public static void preprocess(CommandLine commandLine) throws Exception {
        if (!SparkHC.checkPram(commandLine)) {
            return;
        }

        String readFileName = commandLine.getOptionValue("i");

        Arguments.MODEL_PATH = commandLine.getOptionValue("m");
        Arguments.CONTIG_MAPPING = commandLine.getOptionValue("cm");
        Arguments.RG_MAPPING = commandLine.getOptionValue("rm");
