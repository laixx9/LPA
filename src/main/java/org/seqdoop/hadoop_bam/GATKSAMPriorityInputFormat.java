...
public class GATKSAMPriorityInputFormat
    ...
    public List<InputSplit> getSplits(
        ...
        //double base = splits.size() * 0.09;
        double top1 = splits.size() * 0.01;
        System.out.print("The partition files have been split: ");
        List<PriorityFileSplit> additionSplits = new ArrayList<>();
        for (int ii = 0; ii < splits.size(); ii++) {

            if (Arguments.PARTITIONS_TO_CHANGE.get(ii) >= splits.size()) {
                continue;
            }
            //if(ii<=top1){
            //additionSplits.addAll(getSmallSplits(Arguments.PARTITIONS_TO_CHANGE.get(ii), splits, 16,GATKSAMPriorityInputFormat.VERY_HIGH_LEVEL));
            //}
            //System.out.print("ii:"+ii+","+Arguments.PARTITIONS_TO_CHANGE.get(ii));
            if (ii < Arguments.BASE) {
                //additionSplits.addAll(getSmallSplits(Arguments.PARTITIONS_TO_CHANGE.get(ii), splits, Arguments.ADDITION_EACH_SPLIT_SIZE,GATKSAMPriorityInputFormat.VERY_HIGH_LEVEL));
                additionSplits.addAll(getSmallSplits(Arguments.PARTITIONS_TO_CHANGE.get(ii), splits, Arguments.ADDITION_EACH_SPLIT_SIZE, GATKSAMPriorityInputFormat.VERY_HIGH_LEVEL));
                System.out.print(Arguments.PARTITIONS_TO_CHANGE.get(ii) + ", ");
            } else {
                additionSplits.add(new PriorityFileSplit((FileSplit) splits.get(Arguments.PARTITIONS_TO_CHANGE.get(ii)), GATKSAMPriorityInputFormat.LOW_LEVEL));
            }
        }
